...
void Shutdown()
{
    ...
    RenameThread("CuriusCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: CuriusCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: CuriusCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeCuriusCoinamount=<n> " + _("Keep N CuriusCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CuriusCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CuriusCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CuriusCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeCuriusCoinAmount = GetArg("-anonymizeCuriusCoinamount", 0);
    if(nAnonymizeCuriusCoinAmount > 999999) nAnonymizeCuriusCoinAmount = 999999;
    if(nAnonymizeCuriusCoinAmount < 2) nAnonymizeCuriusCoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeCuriusCoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize CuriusCoin Amount %d\n", nAnonymizeCuriusCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1CuriusCoin+1000 == (.1CuriusCoin+100)*10
       10CuriusCoin+10000 == (1CuriusCoin+1000)*10
    */
